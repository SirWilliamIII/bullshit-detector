...
class BullshitDetectorOCRIntegration {
  ...
  async analyzeImageContent(imagePath, userContext = {}) {
    ...
    try {
      ...
      const ocrResult =
        await this.ocrService.extractTextWithFallback(imagePath);
      ...
      const verificationResult = await this.runTieredVerification(
        ocrResult.text,
        ocrResult,
      );
  ...
  async runTieredVerification(extractedText, ocrResult) {
    ...
    try {
      results.tier1 =
        await this.tier1Sources.verifyAgainstAuthoritySources(extractedText);
    ...
    try {
      results.tier2 =
        await this.tier2Sources.checkComplaintDatabases(extractedText);

      // Multiple Tier 2 sources agreeing = high confidence
      if (
        results.tier2.multipleSourcesConfirm &&
        results.tier2.verdict === "SCAM"
      ) {
        results.finalVerdict = "DEFINITE_SCAM";
    ...
    try {
      results.tier3 = await this.tier3Patterns.analyzeScamPatterns(
        extractedText,
        ocrResult,
      );
    ...
    try {
      results.tier4 =
        await this.tier4Analysis.analyzeBehavioralIndicators(extractedText);
    } catch (error) {
  ...
  async verifyAgainstAuthoritySources(text) {
    ...
    if (lowerText.includes("irs") && lowerText.includes("gmail.com")) {
      return {
        ...
        recommendations: [
          "🚨 IRS never emails about refunds or account issues",
        ],
      };
    }
    ...
    if (
      lowerText.includes("mark zuckerberg") &&
...
class Tier2VerificationService {
  async checkComplaintDatabases(text) {
    ...
    if (
      lowerText.includes("winning amount") &&
      ...
      return {
        ...
        recommendations: [
          "📊 Multiple sources confirm this as a known scam pattern",
        ],
      };
    }
...
class Tier3PatternService {
  async analyzeScamPatterns(text, ocrResult) {
    ...
    return {
      ...
      confidence: strongPatternMatch ? 0.85 : 0.6,
...
class Tier4BehavioralService {
  ...
  checkBusinessModel(text) {
    ...
    return /no.*investment|free.*money|passive.*income.*guaranteed/gi.test(
      text,
    );
  }
}
